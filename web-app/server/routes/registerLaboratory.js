...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        let registeredUser = await databaseHandler.registerNewUser(req.body.registrationId, req.body.name, 'Laboratory');
        res.send("Correct");
    } catch (error) {
...
async function registerInLedger(req) {
    ...
    try {
        ...
        let response = await contract.submitTransaction('createLaboratory', JSON.stringify(req.body));
