...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        let registeredUser = await databaseHandler.registerNewUser(req.body.registrationId, req.body.name, 'Insurance');
        res.send("Correct");
    } catch (error) {
...
async function registerInLedger(req) {
    ...
    try {
        ...
        let response = await contract.submitTransaction('createInsurance', JSON.stringify(req.body));
