...
export default function InsurancePersonalInfo(props) {
  const [updatedData, setUpdatedData] = React.useState(JSON.parse(props.data));
  const [loaded, setLoaded] = React.useState(false);
  var insurance = updatedData;
  insurance.password = "";
  insurance.id = insurance.registrationId;
  ...
  const handleChange = (event) => {
    ...
    console.log(insurance[event.target.name]);
  };
  ...
  const submitForm = async (event) => {
    ...
    try {
      ...
      response = await axios.post(ADDRESS + `updateAsset`, insurance);
