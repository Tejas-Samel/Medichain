...
import ResearcherPersonalInfo from './researcherPersonalInfo';
...
export default function ResearcherDashBoard() {
    ...
    const [researcherData, setResearcherData] = React.useState(researcherFormat);
    const [loaded, setLoaded] = React.useState(false);
    const [open, setOpen] = React.useState(true);
    const [logOut, setLogOut] = React.useState(false);
    const [researcherInfoDisplay, setResearcherInfoDisplay] = React.useState(false);
    ...
    useEffect(() => {
        const fetchResearcherData = async () => {
            setLoaded(true);
            try {
                let researcherCredentials = JSON.parse(localStorage.getItem('researcherToken'));
                console.log(researcherCredentials);
                if (!researcherCredentials) {
                    setLogOut(true);
                } else {
                    researcherCredentials.type = "Researcher";
                    console.log(researcherCredentials);
                    let response = await axios.post(ADDRESS + `readAsset`, researcherCredentials);
                    if (response !== null) {
                        response = response.data;
                        response = JSON.parse(response);
                        response.sessionKey = researcherCredentials.sessionKey;
    ...
    const handleLogOut = async () => {
        ...
        try {
            let researcherCredentials = JSON.parse(localStorage.getItem('researcherToken'));
            researcherCredentials.id = researcherCredentials.registrationId;
            let response = await axios.post(ADDRESS + `logOut`, researcherCredentials);
            response = response.data;
            console.log(response);
            if (response === 'Correct') {
                setLogOut(true);
    ...
    const visibilityHandlerResearcherInfo = () => {
        var x = document.getElementById("researcherInfo");
        if (x) {
            if (x.style.display === "none") {
                x.style.display = "block";
                setResearcherInfoDisplay(true);
            } else {
                x.style.display = "none";
                setResearcherInfoDisplay(false);
            }
        }
    };
    ...
    if (researcherInfoDisplay) {
        console.log("here");
        var researcherInfo = (
            <Paper className={fixedHeightPaper} style={{height: '360'}}>
                <ResearcherPersonalInfo data={JSON.stringify(researcherData)}/>
            </Paper>
        );
        ...
        var requestAccess = (
            <Paper className={fixedHeightPaper} style={{height: '360'}}>
                <RequestDocumentAccess data={JSON.stringify(researcherData)}/>
            </Paper>
        );
        ...
        var viewDocuments = (
            <Paper className={fixedHeightPaper} style={{height: '360'}}>
                <ViewPatientDocuments data={JSON.stringify(researcherData)}/>
            </Paper>
        );
    }
    const mainListItems = (
        <div>
            <ListItem button onClick={visibilityHandlerResearcherInfo}>
                <ListItemIcon>
                    <DashboardIcon/>
                </ListItemIcon>
                <ListItemText primary="ResearcherDashBoard"/>
            </ListItem>
        ...
        <div className={classes.root} id='mainDiv'>
            ...
            <AppBar position="absolute" className={clsx(classes.appBar, open && classes.appBarShift)}>
                <Toolbar className={classes.toolbar}>
                    ...
                    <IconButton color="inherit" onClick={visibilityHandlerResearcherInfo}>
                        <PersonIcon/>
                    </IconButton>
            ...
            <main className={classes.content}>
                ...
                <Container maxWidth="lg" className={classes.container}>
                    ...
                    <Grid container spacing={3} justify='center' alignContent='center'>
                        {/* Recent ResearcherPersonalInfo */}
                        <Grid item xs={12} style={{display: 'none'}} id="researcherInfo">
                            {researcherInfo}
                        </Grid>
